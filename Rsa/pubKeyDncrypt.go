...
func RsaDecryptByPubKey(key, data []byte) ([]byte, error) {

	block, _ := pem.Decode(key)
	if block == nil{
		return nil, errors.New(fmt.Sprintf("pem秘钥校验失败"))
	}

	pubKey, _ := x509.ParsePKIXPublicKey(block.Bytes)
	pk, _ := pubKey.(*rsa.PublicKey)
	ak, err := PublicDecrypt(pk, data)
	if err != nil{
		return nil, errors.New(fmt.Sprintf("公钥解密失败: %v", err))
	}
	return ak, nil
}
